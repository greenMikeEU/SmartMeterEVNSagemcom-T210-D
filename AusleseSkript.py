...
neededConfig = ['port', 'baudrate', 'key', 'printValue', 'useMQTT', 'mqttbrokerip', 'mqttbrokerport',
                'mqttbrokeruser', 'mqttbrokerpasswort', 'useInfluxdb', 'influxdbip', 'influxdbport']
...
# wenn kein User verwendet wird leer lassen ""
mqttuser = config['mqttbrokeruser']
# wenn kein Passwort verwendet wird leer lassen ""
...
if useinfluxdb:
    ...
    try:
        clientinfluxdb = InfluxDBClient(
            host=influxdbhost, port=influxdbport, database=influxdbdatenbank)
...
def evn_decrypt(frame, key, systemTitel, frameCounter):
    """decrypts a frame it using AES encryption"""
                ...
                if value in octet_string_values.keys():
                    if ('Value' in items[i+1].attrib):
                        if value in ['0100010700FF', '0100020700FF']:
                            ...
                            momentan.append(
                                int(items[i+1].attrib['Value'], 16))
                        found_lines.append({'key': octet_string_values[value], 'value': int(
...
#        print(found_lines)
    ...
    if printValue:
        ...
        print(
            "1.0.1.7.0.255\tWirkleistung Bezug [W]: \t "+str(MomentanleistungP))
        print(
            "1.0.2.7.0.255\tWirkleistung Lieferung [W]:\t "+str(MomentanleistungN))
        print("1.0.1.8.0.255\tWirkenergie Bezug [kWh]:\t "+str(WirkenergieP))
        print(
            "1.0.2.8.0.255\tWirkenergie Lieferung [kWh]:\t "+str(WirkenergieN))
        print("-------------\tLeistungsfaktor:\t\t "+str(Leistungsfaktor))
        print(
            "-------------\tWirkleistunggesamt [w]:\t\t " + str(MomentanleistungP-MomentanleistungN))
    ...
    if useMQTT:
        ...
        client.publish("Smartmeter/Wirkleistunggesamt",
